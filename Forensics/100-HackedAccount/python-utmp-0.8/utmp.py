...
class UtmpEntry:
    def __init__(self, *arg, **dictarg):
        ...
        if len(arg)==1:
            arg = arg[0]
        if type(arg) in [tuple, list] and arg:
    ...
    def __getitem__(self, item):
        if item in ['ut_type', 0]:
            return self.ut_type
        elif item in ['ut_pid', 1]:
            return self.ut_pid
        elif item in ['ut_line', 2]:
            return self.ut_line
        elif item in ['ut_id', 3]:
            return self.ut_id
        elif item in ['ut_user', 4]:
            return self.ut_user
        elif item in ['ut_host', 5]:
            return self.ut_host
        elif item in ['ut_exit', 6]:
            return self.ut_exit
        elif item in ['ut_session', 7]:
            return self.ut_session
        elif item in ['ut_tv', 8]:
            return self.ut_tv
        elif item in ['ut_addr_v6', 9]:
            return self.ut_addr_v6
        else:
            raise IndexError(f"Bad key used to access UtmpEntry: {repr(item)}")
...
class UtmpRecord:
    ...
    def _makeclass(self, a):
        return UtmpEntry(a) if a else None
    ...
    def __repr__(self):
        if not self.fname:
            fname = ''
        return f'utmp.UtmpRecord({fname})'
